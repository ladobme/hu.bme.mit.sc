...
public class AllItemMatcher extends BaseMatcher<AllItemMatch> {
   ...
   * 
   */
  public static AllItemMatcher on(final ViatraQueryEngine engine) throws ViatraQueryException {
    // check if matcher already exists
    AllItemMatcher matcher = engine.getExistingMatcher(querySpecification());
    if (matcher == null) {
    	matcher = (AllItemMatcher)engine.getMatcher(querySpecification());
    }
    return matcher;
  }
   ...
   * 
   */
  public static AllItemMatcher create() throws ViatraQueryException {
    return new AllItemMatcher();
  }
  
  private final static int POSITION_EOBJECT = 0;
  
  private final static Logger LOGGER = ViatraQueryLoggingUtil.getLogger(AllItemMatcher.class);
   ...
   * 
   */
  private AllItemMatcher() throws ViatraQueryException {
  ...
  protected AllItemMatch tupleToMatch(final Tuple t) {
    try {
    	return AllItemMatch.newMatch((EObject) t.get(POSITION_EOBJECT));
  ...
  protected AllItemMatch arrayToMatch(final Object[] match) {
    try {
    	return AllItemMatch.newMatch((EObject) match[POSITION_EOBJECT]);
    } catch(ClassCastException e) {
  ...
  protected AllItemMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return AllItemMatch.newMutableMatch((EObject) match[POSITION_EOBJECT]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
   ...
   * 
   */
  public static IQuerySpecification<AllItemMatcher> querySpecification() throws ViatraQueryException {
